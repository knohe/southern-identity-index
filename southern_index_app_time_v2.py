...
st.set_page_config(page_title="Southern Identity Index — Time Slider (Header-Tolerant)", layout="wide")
st.title("Southern Identity Index — Time Slider (Header-Tolerant)")

st.markdown(
    "This version auto-fixes common CSV header typos (e.g., **historical_continutity** → **historical_continuity**). "
    "Upload a time-series CSV or use the example, then move the **time slider** to see motion and blobs."
)
...
# -----------------------
# Data Upload & Header Normalization
# -----------------------
st.header("1) Upload or Use Example Data")
...
uploaded = st.file_uploader("Upload CSV with columns including 'name' and 'year'", type=["csv"])

def normalize_headers(df):
    # mapping of normalized -> canonical
    canonical = {
        "name": "name",
        "year": "year",
        "econ_x": "econ_x",
        ...
        "racial_disparities": "racial_disparities",
        "political_culture": "political_culture",
        "cultural_identity": "cultural_identity",
        "historical_continuity": "historical_continuity",
    }
    # known common typos / variants
    aliases = {
        "historical_continutity": "historical_continuity",  # typo
        "historical_continunity": "historical_continuity",
        "historicalcontinuity": "historical_continuity",
        "class_and_inequality": "class_inequality",
        "classinequality": "class_inequality",
        "racial_disparity": "racial_disparities",
        "racialdisparities": "racial_disparities",
    ...
    for col in df.columns:
        ...
        if key in canonical:
            new_cols[col] = canonical[key]
        elif key in aliases:
            new_cols[col] = aliases[key]
        else:
            # leave as-is; we might not need it
            new_cols[col] = col
...
required = [
    "name","year","econ_x","hier_y","economic","class_inequality",
    "racial_disparities","political_culture","cultural_identity","historical_continuity"
]
...
interpolate = st.sidebar.checkbox("Interpolate between years", value=True, key="interp2")
show_trails = st.sidebar.checkbox("Show trails (paths across years)", value=True, key="trail2")
        ...
        if not interpolate:
            ...
            if earlier.empty:
                ...
                if r0["year"] == r1["year"]:
                    ...
                    vals = {}
                    for c in ["econ_x","hier_y","economic","class_inequality","racial_disparities",
                              "political_culture","cultural_identity","historical_continuity","SII"]:
                        vals[c] = (1 - t) * r0[c] + t * r1[c]
                    row = pd.Series({"name": name, "year": year, **vals})
                    out_rows.append(row)
...
# Scatter + trails
# -----------------------
st.header("3) Quadrant Scatter (Snapshot)")
...
if not snap.empty:
    sizes = (snap["SII"] * 10.0).clip(lower=20.0, upper=1000.0)
